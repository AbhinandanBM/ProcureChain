...
def maketenderback():
    if request.method=='POST':
        ...
        dd = r"D:\Blockchain based e-Tendering System\Updated CODE\CODE\text_files/" + file
        print(dd)
        f = open(dd, "r")
        data = f.read()
        now = datetime.now()
...
def viewnot():
    ...
    if today > yesterday:
        mydb,cursor=get_db()
        sql ="select id,email,obj,cost,sdate,edate from notifications where email='%s'" %(email)
        x = pd.read_sql_query(sql, mydb)
        x = x.drop(['email'], axis=1)
...
def finalised(s=0,s1=''):
    ...
    if count==1:
        ...
        otp = "congratulations for woning this tender. Now u can start the payment Process"
        t = 'Regards,'
        t1 = 'Tender Officer.'
        mail_content = msg + '\n' + otp +'\n' + '\n' + t + '\n' + t1
        sender_address = 'smart.etender65@gmail.com'
        sender_pass = 'sbvcjnjarsuazuxr'
        receiver_address = s1
        message = MIMEMultipart()
        message['From'] = sender_address
        message['To'] = receiver_address
        message['Subject'] = 'Blockchain Based E-Tendering System'
        ...
        sql1 = "update notifications set payment='accepted' where id='%s'"%(s)
...
def payment():
    if request.method =="POST":
        ...
        mydb,cursor=get_db()
        sql = "insert into payment(name,Email,amount,cardname,cardnumber,cvv)values(%s,%s,%s,%s,%s,%s)"
        val = (name,Email,amount,cardname,cardnumber,cvv)
        cursor.execute(sql,val)
        mydb.commit()
...
def viewpayment():
    ...
    if today > yesterday:
        ...
        x = pd.read_sql_query(sql, mydb)
        return render_template('viewpayment.html', cal_name=x.columns.values, row_val=x.values.tolist())
    else:
        flash("No tenders are available","warning")
